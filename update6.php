...
<?php }
 ?>
        ...
        <div class="kol">
            <div class="sign">
            <span class="subhead"><center><h3>III B.Sc Chemistry Students</h3></center></span>
    <center>
...
$sql=mysqli_query($conn,"SELECT * FROM `3rdbsc(c)` WHERE id LIKE '$id'");
...
if(isset($_POST['register']))
{
    ...
    {
        $sql=mysqli_query($conn,"UPDATE `3rdbsc(c)` SET `Name`='$nam',`Regno`='$roll',`Class`='$cls',`Phone`='$deg' WHERE id LIKE '$id'");
        if($sql)
        {
            ?>
...
<center><h7><a href="http://localhost/CHEMISTRYSTORE/IIIbsc(c)view.php" style="color:#303030; font-size: 12px;">Go Back</a></h7></center>
            </div>
        </div>
