...
if(isset($_GET['id'])){
	$d=$_GET['id'];
	$sql=mysqli_query($conn,"DELETE FROM `entrylrchemicals` WHERE id='$d'");
	if($sql)
	{
		?>
...
<div class="navss">
    ...
    <span class="subhead"><center><h3>Total Stocks of LR Chemicals</h3></center></span>
    <div class="contai mt-5">
        <div class="rw justify-content-center">
            <div class="col-md-8">
                <form action="">
               <div class="input-group mb-3">
                <input type="text" class="form-control" value="<?php if(isset($_GET['search'])){echo $_GET['search'];}?>" required="required" name="search" placeholder=" Search Brands,Product Names,Quantitys,Rates,Ml|Kg,Chemicals,Apparatus">
    ...
    <center>
<table border="1" style="background: white;" cellpadding="5" cellspacing="5" class="design" align="center" class="design">
<tr  style="color: #fff; background-color:#303030; text-align:left; font-size:16px;"><b><th>IMAGE</th><th>PRODUCT NAME</th><th>BRAND</th><th>QUANTITY</th><th>ML|KG</th><th>RATE</th><th>UPDATE</th><th>+CART</th><th>DELETE</th></b></tr>
<?php 
if(isset($_GET['search'])){
    $filter=$_GET['search'];
    $filter2="SELECT * FROM `entrylrchemicals` WHERE CONCAT(Brand,Quantity,Rate,mlkg,Name) LIKE '%$filter%'";
    ...
    if(mysqli_fetch_array($filtrow) > 0){
        foreach($filtrow as $row){
            ?>
                ...
                <td><a href="update3.php?id=<?php echo $row['id'];?>" class="fg1" style="color: white; text-align:center; margin-left:10px;">Update</a></td>
                <td><a href="cart3.php?id=<?php echo $row['id'];?>"class="fg14" style="color: white; text-align:center; margin-left:10px;">+ Cart</a></td>
                <td><a href="viewlrchemicals.php?id=<?php echo $row['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px; text-align:center; margin-left:10px;">Remove</a></td>
            </tr>
            <?php
        }
    }
...
$i=1;
$x=mysqli_query($conn,"SELECT * FROM `entrylrchemicals`");
while($y=mysqli_fetch_array($x))
{
	?>
<tr style="font-size:17px;color:black; ">
<!-- <td><?php echo $i++;?></td> -->
<td><?php echo "<img src='$y[Image]'  height=80px width=80px />" ;?></td>
<td><?php echo $y['Name'];?></td>
<td><?php echo $y['Brand'];?></td>
<td><?php echo $y['Quantity'];?></td>
<td><?php echo $y['mlkg'];?></td>
<td><?php echo $y['Rate'];?></td>
<td><a href="update3.php?id=<?php echo $y['id'];?>" class="fg1" style="color: white; text-align:center; margin-left:10px;">Update</a></td>
<td><a href="cart3.php?id=<?php echo $y['id'];?>"class="fg14" style="color: white; text-align:center; margin-left:10px;">+ Cart</a></td>
<td><a href="viewlrchemicals.php?id=<?php echo $y['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px; text-align:center; margin-left:10px;">Remove</a></td>
</tr>
<?php }
 }?>
</table>
<button id="back-to-top" onclick="topFunction()" title="Go to top">&#8679;</button>
    ...
    <center><h5><a href="http://localhost/CHEMISTRYSTORE/viewlrchemicals.php">ALL</a></h5></center>
    </div>
