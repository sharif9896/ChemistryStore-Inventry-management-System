...
if(isset($_GET['id'])){
	$d=$_GET['id'];
	$sql=mysqli_query($conn,"DELETE FROM `3rdbsc(c)` WHERE id='$d'");
	if($sql)
	{
		?>
...
<div class="navss">
    ...
    <span class="subhead"><center><h3>III B.Sc Chemistry</h3></center></span>
...
if(isset($_GET['search'])){
    $filter=$_GET['search'];
    $filter2="SELECT * FROM `3rdbsc(c)` WHERE CONCAT(Regno,Name,Class,Phone) LIKE '%$filter%'";
    $x=mysqli_query($conn,"SELECT * FROM `3rdbsc(c)` ORDER BY Regno ASC");
    ...
    if(mysqli_fetch_array($filtrow) > 0){
        foreach($filtrow as $row){
            ?>
                ...
                <td><a href="update6.php?id=<?php echo $row['id'];?>" class="fg12" style="color: white;">Update</a></td>
                <td><a href="IIIbsc(c)view.php?id=<?php echo $row['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px;">Remove</a></td>
                </tr>
            <?php
        }
    }
...
$i=1;
$x=mysqli_query($conn,"SELECT * FROM `3rdbsc(c)`");
$x=mysqli_query($conn,"SELECT * FROM `3rdbsc(c)` ORDER BY Regno ASC");
...
<td><a href="update6.php?id=<?php echo $y['id'];?>" class="fg12" style="color: white;">Update</a></td>
<td><a href="IIIbsc(c)view.php?id=<?php echo $y['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px;">Remove</a></td>
</tr>
<?php }
 }?>
</table>
