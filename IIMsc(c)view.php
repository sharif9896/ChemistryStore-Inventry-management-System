...
if(isset($_GET['id'])){
	$d=$_GET['id'];
	$sql=mysqli_query($conn,"DELETE FROM `2ndmsc(c)` WHERE id='$d'");
	if($sql)
	{
		?>
...
<div class="navss">
    ...
    <span class="subhead"><center><h3>II M.Sc Chemistry</h3></center></span>
...
if(isset($_GET['search'])){
    $filter=$_GET['search'];
    $filter2="SELECT * FROM `2ndmsc(c)` WHERE CONCAT(Regno,Name,Class,Phone) LIKE '%$filter%'";
    $x=mysqli_query($conn,"SELECT * FROM `2ndmsc(c)` ORDER BY Regno ASC");
    ...
    if(mysqli_fetch_array($filtrow) > 0){
        foreach($filtrow as $row){
            ?>
                ...
                <td><a href="update8.php?id=<?php echo $row['id'];?>" class="fg12" style="color: white;">Update</a></td>
                <td><a href="IIMsc(c)view.php?id=<?php echo $row['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px;">Remove</a></td>
                </tr>
            <?php
        }
    }
...
$i=1;
$x=mysqli_query($conn,"SELECT * FROM `2ndmsc(c)`");
$x=mysqli_query($conn,"SELECT * FROM `2ndmsc(c)` ORDER BY Regno ASC");
...
<td><a href="update8.php?id=<?php echo $y['id'];?>" class="fg12" style="color: white;">Update</a></td>
<td><a href="IIMsc(c)view.php?id=<?php echo $y['id'];?>" class="fe2" style="color: white;  padding: 10px 10px; font-size: 15px;">Remove</a></td>
</tr>
<?php }
 }?>
</table>
<button id="back-to-top" onclick="topFunction()" title="Go to top">&#8679;</button>
    ...
    <center><h5><a href="http://localhost/CHEMISTRYSTORE/IMsc(c).php">ALL</a></h5></center>
    </div>
