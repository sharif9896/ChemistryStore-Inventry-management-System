...
if(isset($_POST['register']))
{
    ...
    {
        $x=mysqli_query($conn,"SELECT * FROM entryproducts WHERE Productname LIKE '$pro'");
        $y=mysqli_fetch_array($x);
        $qutn=$y['Quantity'];
        $rat=$y['Rate'];
        $x=mysqli_query($conn,"SELECT * FROM studentinfo WHERE Name LIKE '$nam'");
        $y=mysqli_fetch_array($x);
        $em=$y['Email'];
        $ps=$y['Password'];
        $sql=mysqli_query($conn,"INSERT INTO `producttaken`(`Regno`, `Productname`, `Date`, `Time`,`Quantity`,`Rate`,`Name`,`Email`,`Password`) VALUES (' $roll','$pro',NOW(),NOW(),'$qutn','$rat','$nam','$em','$ps')");
        
        if($sql)
        {
            echo "Successfully Taken!";
        }
        else
        {
