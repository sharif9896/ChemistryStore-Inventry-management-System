...
<?php }
 ?>
        ...
        <div class="kol">
            <div class="sign">
            <span class="subhead"><center><h3>Product Taken</h3></center></span>
    <center>
...
if(isset($_POST['register']))
{
    ...
    else
    {
        ...
        $sql=mysqli_query($conn,"INSERT INTO `protaken`(`Brand`, `Quantity`, `mlkg`, `Rate`, `Date`, `Time`,`Name`,`Regno`,`stud`) VALUES ('$brd','$Quan','$mlk','$rate',NOW(),NOW(),'$nam','$roll','$nmm')");
        if($sql)
        {
            ?>
            <div class="mi" style="color:#303030; font-size: 13px;">Successfully Taken!</div>
        ...
        if($cc==0){
            $sql=mysqli_query($conn,"UPDATE `entryproducts` SET `Brand`='$brd',`Quantity`='$cc',`mlkg`='$mlk',`Rate`='$rate',`Name`='$nam' WHERE Name LIKE '$nam'");
        ...
        $c11=mysqli_query($conn,"SELECT * FROM `entryarchemicals` WHERE Name LIKE '$nam'");
        $rss=mysqli_fetch_array($c11);
        $fii=$rss['Quantity'];

        $cc1=$fii-$Quan;

        if($cc1==0){
            $sql=mysqli_query($conn,"DELETE FROM `entryarchemicals` WHERE Name LIKE '$nam'");
        }
        else{
            $sql=mysqli_query($conn,"UPDATE `entryarchemicals` SET `Brand`='$brd',`Quantity`='$cc1',`mlkg`='$mlk',`Rate`='$rate',`Name`='$nam' WHERE Name LIKE '$nam'");
        }
        
        $c11=mysqli_query($conn,"SELECT * FROM `entrysrchemicals` WHERE Name LIKE '$nam'");
        $rss=mysqli_fetch_array($c11);
        $fii=$rss['Quantity'];
        ...
        if($cc1==0){
            $sql=mysqli_query($conn,"DELETE FROM `entrysrchemicals` WHERE Name LIKE '$nam'");
        }
        else{
            $sql=mysqli_query($conn,"UPDATE `entrysrchemicals` SET `Brand`='$brd',`Quantity`='$cc1',`mlkg`='$mlk',`Rate`='$rate',`Name`='$nam' WHERE Name LIKE '$nam'");
        }

        $c11=mysqli_query($conn,"SELECT * FROM `entrylrchemicals` WHERE Name LIKE '$nam'");
        $rss=mysqli_fetch_array($c11);
        $fii=$rss['Quantity'];
        ...
        if($cc1==0){
            $sql=mysqli_query($conn,"DELETE FROM `entrylrchemicals` WHERE Name LIKE '$nam'");
        }
        else{
            $sql=mysqli_query($conn,"UPDATE `entrylrchemicals` SET `Brand`='$brd',`Quantity`='$cc1',`mlkg`='$mlk',`Rate`='$rate',`Name`='$nam' WHERE Name LIKE '$nam'");
        }
        // if($cc){
...
<tr><td colspan="2" align="center"><input type="submit" value="Taken" name="register" class="fields fg" /></td></tr>
</table>
</form>
<center><h5><a href="http://localhost/CHEMISTRYSTORE/dashboard.php" style="color:#303030; font-size: 12px;">Dashboard</a></h5></center>
            </div>
