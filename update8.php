...
<?php }
 ?>
        ...
        <div class="kol">
            <div class="sign">
            <span class="subhead"><center><h3>II M.Sc Chemistry Students</h3></center></span>
    <center>
...
$sql=mysqli_query($conn,"SELECT * FROM `2ndmsc(c)` WHERE id LIKE '$id'");
...
if(isset($_POST['register']))
{
    ...
    {
        $sql=mysqli_query($conn,"UPDATE `2ndmsc(c)` SET `Name`='$nam',`Regno`='$roll',`Class`='$cls',`Phone`='$deg' WHERE id LIKE '$id'");
        if($sql)
        {
            ?>
...
<center><h7><a href="http://localhost/CHEMISTRYSTORE/IIMsc(c)view.php" style="color:#303030; font-size: 12px;">Go Back</a></h7></center>
            </div>
        </div>
