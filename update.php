...
<?php }
 ?>
        ...
        <div class="kol">
            <div class="sign">
            <span class="subhead"><center><h3>Update Apparatus</h3></center></span>
    <center>
...
$sql=mysqli_query($conn,"SELECT * FROM `entryproducts` WHERE id LIKE '$id'");
...
if(isset($_POST['register']))
{
    ...
    foreach($sql as $full){
        ...
        $sql=mysqli_query($conn,"UPDATE `entryproducts` SET `Brand`='$brd',`Quantity`='$Quan',`mlkg`='$ml',`Rate`='$rat',`Name`='$nam',`Image`='$oldimg' WHERE id LIKE '$id'");
        if($sql)
        {
            ?>
    ...
    if(isset($_POST['register']))
{
    ...
    else 
    {
        ...
        $sql=mysqli_query($conn,"UPDATE `entryproducts` SET `Brand`='$brd',`Quantity`='$Quan',`mlkg`='$ml',`Rate`='$rat',`Name`='$nam',`Image`='$folder' WHERE id LIKE '$id'");
        if($sql)
        {
            ?>
            <div class="mi" style="color:#303030; font-size: 13px;">Updated Successfully!</div>
...
<center><h7><a href="http://localhost/CHEMISTRYSTORE/viewaapa.php" style="color:#303030; font-size: 12px;">Go Back</a></h7></center>
            </div>
        </div>
